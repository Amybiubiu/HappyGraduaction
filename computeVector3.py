...
while True:
    ...
    # 将 txt 转为 json
    item = {}
    item['source'], item['target'], item['value'] = author1, author2, collaboration
    jsonData.append(item)
import json
with open('result/edges.json', 'w', encoding='utf-8') as f:
    json.dump(jsonData, f, ensure_ascii=False, indent=4)
print(f'edges: {len(jsonData)}, row: {rowid-nodes+2-1}')
